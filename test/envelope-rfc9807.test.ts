...
describe('Envelope RFC 9807 Compliance', () => {
  ...
  it('should store and recover envelope', async () => {
    ...
    const { client_private_key, export_key: recovered_export_key } = recoverResult.unsafeCoerce()
  ...
  it('should fail recovery with modified envelope', async () => {
    ...
    const recoverResult = await ops.recover(randomized_pwd, modifiedEnvelope, creds, mockDeriveKeyPair)
    expect(recoverResult.isLeft()).toBe(true)
  })
  ...
  it('should fail recovery with modified cleartext credentials', async () => {
    ...
    const recoverResult = await ops.recover(randomized_pwd, envelope, wrongCreds, mockDeriveKeyPair)
    expect(recoverResult.isLeft()).toBe(true)
  })
