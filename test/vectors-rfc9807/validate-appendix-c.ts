...
type IndexEntry = { readonly section: string; readonly name: string; readonly file: string }
...
if (!existsSync(rfcPath)) {
    ...
    console.error(
        'Download with: curl -L https://www.rfc-editor.org/rfc/rfc9807.txt -o /tmp/rfc9807.txt'
    )
    process.exit(1)
}

const index = JSON.parse(readFileSync(INDEX_PATH, 'utf8')) as {
    readonly sections: readonly IndexEntry[]
}
...
function parseRFCAppendixC(text: string): ParsedRFC {
    ...
    for (const line of lines) {
        ...
        const areaMatch = line.match(
            /^C\.\d+\.\d+\.\d+\.\s+(Configuration|Input Values|Intermediate Values|Output Values)/
        )
        if (areaMatch) {
        ...
        if (fieldMatch) {
            ...
            if (currentArea === 'config') {
                sections[currentSection][currentArea][field] = /^\d+$/.test(value)
                    ? parseInt(value, 10)
                    : value
            } else {
                sections[currentSection][currentArea][field] = value.replace(/\s+/g, '')
            }
            continue
        }
        ...
        if (currentField && /^\s{3}\S+/.test(line)) {
            ...
            if (currentArea !== 'config') {
                const areaObj = sections[currentSection][currentArea]
    ...
    for (const area of ['config', 'inputs', 'intermediates', 'outputs'] as const) {
        ...
        for (const key of keys) {
            ...
            if (typeof expected === 'number' || typeof actual === 'number') {
                if (expected !== actual) {
                    report(
                        `[${section}] ${area}.${key} config mismatch: RFC=${expected} local=${actual}`
                    )
                    failed = true
                }
                continue
            ...
            if (expected !== actual) {
                ...
                report(
                    `[${section}] ${area}.${key} mismatch (RFC len=${expected.length}, local len=${actual.length}, firstDiffIndex=${idx})`
                )
                failed = true
            }
        }
...
function validateLengths(section: string, vector: VectorSection): boolean {
    ...
    const lengthChecks: ReadonlyArray<readonly [string, number | undefined]> = [
    ...
    for (const [label, actualLen] of lengthChecks) {
        if (expected[label] !== undefined) {
            if (!actualLen) {
                ...
                report(
                    `[${section}] ${label} length mismatch: expected ${expected[label]} got ${actualLen}`
                )
                failed = true
            }
        }
