...
export const deriveKeys = curry3(
    ...
    > => {
        return tryCatchAsync(async () => {
            ...
            // handshake_secret = Derive-Secret(prk, "HandshakeSecret", Hash(preamble))
            const handshakeSecretResult = await deriveSecret(config)(prk)(LABELS.HandshakeSecret)(
                preamble_hash
            )
            ...
            // session_key = Derive-Secret(prk, "SessionKey", Hash(preamble))
            const sessionKeyResult = await deriveSecret(config)(prk)(LABELS.SessionKey)(
                preamble_hash
            )
            ...
            // Km2 = Derive-Secret(handshake_secret, "ServerMAC", "")
            const Km2Result = await deriveSecret(config)(handshake_secret)(LABELS.ServerMAC)(
                new Uint8Array(0)
            )
            ...
            // Km3 = Derive-Secret(handshake_secret, "ClientMAC", "")
            const Km3Result = await deriveSecret(config)(handshake_secret)(LABELS.ClientMAC)(
                new Uint8Array(0)
            )
