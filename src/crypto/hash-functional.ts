...
export const hashMessage = curry2(
  async (
    ...
    return tryCatchAsync(async () => {
      opaqueLogger.debug({ message: `Hashing message with ${config.name}`, algorithm: config.name, messageLength: message.length })
...
export interface BoundHashOps {
  hash: (message: Uint8Array) => Promise<Either<Error, Uint8Array>>
  config: HashConfig
}
