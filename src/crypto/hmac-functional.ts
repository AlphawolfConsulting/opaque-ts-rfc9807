...
export interface HMACOps {
  sign: (message: Uint8Array) => Promise<Either<Error, Uint8Array>>
  verify: (message: Uint8Array) => (tag: Uint8Array) => Promise<Either<Error, boolean>>
}
...
const importHMACKey = curry2(
  async (
    config: HMACConfig,
    key: Uint8Array
  ): Promise<Either<Error, CryptoKey>> => {
    return tryCatchAsync(async () => {
      opaqueLogger.debug({ message: 'Importing HMAC key', hash: config.hash, keyLength: key.length })
...
const hmacSign = curry2(
  async (
    ...
    return tryCatchAsync(async () => {
      ...
      opaqueLogger.debug({ message: 'HMAC signature generated', signatureLength: result.length })
...
const hmacVerify = curry3(
  async (
    ...
    return tryCatchAsync(async () => {
      opaqueLogger.debug({ message: 'Verifying HMAC tag', messageLength: message.length, tagLength: tag.length })
...
export interface BoundHMACOps {
  withKey: (key: Uint8Array) => Promise<Either<Error, HMACOps>>
  config: HMACConfig
}
