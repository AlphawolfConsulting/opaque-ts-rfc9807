...
export const createRegistrationRequest = curry2(
    async (
        ...
        return tryCatchAsync(async () => {
            ...
            opaqueLogger.debug({
                message: 'Creating registration request',
                requestId,
...
export const createRegistrationResponse = curry4(
    async (
        ...
        return tryCatchAsync(async () => {
            ...
            const expandResult = await kdf.expand(oprf_seed)(new Uint8Array(LABELS.OprfKey))(
...
export const finalizeRegistrationRequest = curry8(
    async (
        ...
        return tryCatchAsync(async () => {
            ...
            opaqueLogger.debug({
                message: 'Finalizing registration request',
                recordId,
                config: config.hash.name
            })
            ...
            const cleartextCreds = createCleartextCredentials(response.server_public_key)(
                server_identity
            )(client_identity)

            // Store envelope - SIDE EFFECT: Crypto RNG for nonce
            const envelopeOps = createEnvelopeOps(config)
            const storeResult = await envelopeOps.store(
                randomized_pwd,
                cleartextCreds,
                deriveKeyPair
            )
            ...
            const { envelope, client_public_key, masking_key, export_key } =
                storeResult.unsafeCoerce()
...
export const createRegistrationOps = (config: Config): BoundRegistrationOps => ({
    ...
    finalizeRequest: (
        ...
        finalizeRegistrationRequest(config)(password)(blind)(response)(server_identity)(
            client_identity
        )(ksf)(deriveKeyPair)
})
