...
export const createCredentialRequest = curry3(
    async (
        config: Config,
        password: Uint8Array,
        generateKeyshare: () => Promise<
            Either<Error, { readonly keyshare: Uint8Array; readonly private: Uint8Array }>
        >
    ): Promise<
        Either<
            Error,
            { readonly ke1: KE1; readonly blind: Uint8Array; readonly client_secret: Uint8Array }
        >
    > => {
        return tryCatchAsync(async () => {
            ...
            const { keyshare: client_keyshare, private: client_secret } =
                keyshareResult.unsafeCoerce()
...
export const maskCredentialResponse = curry4(
    async (
        ...
        data: {
            ...
            readonly envelope: { readonly nonce: Uint8Array; readonly auth_tag: Uint8Array }
        }
...
export const unmaskCredentialResponse = curry3(
    ...
    > => {
        return tryCatchAsync(async () => {
            ...
            const padInfo = new Uint8Array([
                ...credential_response.masking_nonce,
                ...LABELS.CredentialResponsePad
            ])
            const padLength = config.ake.Npk + config.constants.Nn + config.mac.Nm // server_public_key + envelope
            ...
            const envelope_nonce = unmasked.slice(
                config.ake.Npk,
                config.ake.Npk + config.constants.Nn
            )
    ...
    > => {
        return tryCatchAsync(async () => {
            ...
            const oprfOutputResult = await oprfOps.finalize({
                input: password,
                blind,
                evaluation: ke2.credential_response.data
            })
            ...
            // Derive masking key
            const maskingKeyResult = await kdf.expand(randomized_pwd)(
                new Uint8Array(LABELS.MaskingKey)
            )(config.hash.Nh)
            ...
            const unmaskedResult = await unmaskCredentialResponse(config)(masking_key)(
            ...
            const recoverResult = await envelopeOps.recover(
                randomized_pwd,
                envelope,
                cleartextCreds,
                deriveKeyPair
...
export const createCredentialResponse = curry5(
    async (
        ...
        envelope: { readonly nonce: Uint8Array; readonly auth_tag: Uint8Array },
        oprf_key: Uint8Array
    ): Promise<
        Either<
            Error,
            {
                readonly credential_response: KE2['credential_response']
    ...
    > => {
        return tryCatchAsync(async () => {
            opaqueLogger.debug({
                message: 'Creating credential response (KE2)',
                sessionId: ke1.sessionId
            })
            ...
            const maskedResult = await maskCredentialResponse(config)(masking_key)(masking_nonce)({
                server_public_key,
                envelope
            })
...
export interface BoundAuthOps {
    readonly createCredentialRequest: (
        password: Uint8Array
    ) => (
        generateKeyshare: () => Promise<
            Either<Error, { readonly keyshare: Uint8Array; readonly private: Uint8Array }>
    ...
    readonly recoverCredentials: (password: Uint8Array) => (blind: Uint8Array) => (ke2: KE2) => (
        server_identity: Uint8Array
    ) => (client_identity: Uint8Array) => (
        deriveKeyPair: (
            seed: Uint8Array
    ...
    readonly createCredentialResponse: (ke1: KE1) => (server_public_key: Uint8Array) => (envelope: {
        readonly nonce: Uint8Array
        readonly auth_tag: Uint8Array
    }) => (oprf_key: Uint8Array) => Promise<
        Either<
            Error,
            {
                readonly credential_response: KE2['credential_response']
    ...
    readonly unmaskCredentialResponse: (masking_key: Uint8Array) => (credential_response: {
        readonly masking_nonce: Uint8Array
        readonly masked_response: Uint8Array
    }) => Promise<
        Either<
    ...
    readonly maskCredentialResponse: (
        masking_key: Uint8Array
    ) => (
        masking_nonce: Uint8Array
    ) => (data: {
